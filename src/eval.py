  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    for i in xrange(num_images):
      ...
      det_boxes, det_probs, det_class, det_conf = sess.run(
           [model.det_boxes, model.det_probs, model.det_class, model.pred_conf],
           feed_dict={model.image_input:images})
      _t['im_detect'].toc()
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    imdb = kitti_test.kitti_test(FLAGS.image_set, FLAGS.data_path, mc)
