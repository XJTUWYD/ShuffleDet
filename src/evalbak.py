...
from utils import float2pow2 as f2p
from nets import *
...
tf.app.flags.DEFINE_string('activations_path', './activations_path', """activation path""")
  ...
  with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
    ...
    for i in xrange(num_images):
      ...
      det_boxes, det_probs, det_class, det_conf,preds= sess.run(
           [model.det_boxes, model.det_probs, model.det_class, model.pred_conf,model.preds],
           feed_dict={model.image_input:images})
      ################ save feature map #######################
      image_input,conv1, pool1, stage_1_1,left_a, left_b, right_a, right_b,right_c = sess.run(
           [model.image_input,model.conv1, model.pool1, model.stage_1_1,model.left_a, model.left_b, model.right_a, model.right_b, model.right_c],
           feed_dict={model.image_input:images})
      _t['im_detect'].toc()
##############print the feature map##############
      ...
      ratio_list = [conv1_ratio, pool1_ratio, left_a_ratio, left_b_ratio, right_a_ratio, right_b_ratio, right_c_ratio]
...
def main(argv=None):  # pylint: disable=unused-argument
  ...
  if tf.gfile.Exists(FLAGS.activations_path):
    tf.gfile.DeleteRecursively(FLAGS.activations_path)
  tf.gfile.MakeDirs(FLAGS.activations_path)
  evaluate()
