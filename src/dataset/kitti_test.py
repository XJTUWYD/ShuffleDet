...
from utils.util import bbox_transform_inv, batch_iou, dac_iou

class kitti_test(imdb):
  def __init__(self, image_set, data_path, mc):
    ...
    self._image_path = os.path.join(self._data_root_path, 'testing', 'image_2')
    self._label_path = os.path.join(self._data_root_path, 'testing', 'label_2')
  ...
  def analyze_detections(self, detection_file_dir, det_error_file):
    ...
    with open(det_error_file, 'w') as f:
      all_det_bboxes = np.concatenate(dict.values(self._det_rois))
      all_gt_bboxes = np.concatenate(dict.values(self._rois))
      all_ious = dac_iou(all_gt_bboxes, all_det_bboxes)
      print len(all_det_bboxes)
      print len(all_gt_bboxes)
