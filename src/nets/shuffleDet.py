...
class shuffleDet(ModelSkeleton):
  ...
  def _add_forward_graph(self):
    mc = self.mc
    self.conv1 = self._conv_layer('conv1', self.image_input, filters=24, size=3, stride=2, padding='SAME', freeze=False)
    self.pool1 = self._pooling_layer('pool1', self.conv1 , size=2, stride=2, padding='SAME')
##########################ShuffleNet####################################################
    # #module d (repeat:1)
    ...
    self.preds = self._conv_layer('preds', self.stage_6_3 , filters=num_output, size=3, stride=1,padding='SAME', xavier=False, relu=False, stddev=0.0001)
